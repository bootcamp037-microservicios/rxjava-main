...
public class Main {
    ...
    void metodosObservable(){
        Observable.just(1, 3, 5, 4)
            .doOnNext(v -> {
                if (v == 4){
                    throw new RuntimeException("El cuatro tiene error: ");
                }
            })
    ...
    void operadorFlapMap(){
        ...
        ob01
            //.doOnNext(x -> System.out.println(" obs01: " + x))
            .flatMap(x -> ob02.map(v -> String.format("nuevo %s", v)))
            .subscribe(System.out::println);

        //ob02.subscribe(System.out::println);
    }

    public static void main(String[] args) {
        Main app = new Main();
        //app.operadores();
        //app.metodosObservador();
